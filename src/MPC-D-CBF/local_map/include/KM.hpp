...
public:
  ...
  void setMatchThreshold(double dist) { match_threshold_ = dist; }
  void setHistoryThreshold(double dist) { history_threshold_ = dist; }
...
private:
  ...
  double match_threshold_ = 1.0;
  double history_threshold_ = 0.2;
};
...
void KMAlgorithm::tracking(std::vector<Ellipse>& input_vector)
{
  ...
  else if (new_size > 0)
  {
    ...
    {
      if (assignment[i] != -1 && calculate_dis(input_vector[i], last_label_list[assignment[i]]) < match_threshold_)
