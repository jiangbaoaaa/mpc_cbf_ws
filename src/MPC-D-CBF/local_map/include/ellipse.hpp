...
inline bool is_valid_ellipse(const Ellipse &ellipse)
{
  return std::isfinite(ellipse.cx) && std::isfinite(ellipse.cy) &&
         std::isfinite(ellipse.semimajor) && std::isfinite(ellipse.semiminor) &&
         std::isfinite(ellipse.theta) &&
         ellipse.semimajor > 0.0 && ellipse.semiminor > 0.0;
}
...
vector<Ellipse> get_ellipse_array(std::vector<Obstacle> &seg_obs, const grid_map::GridMap &localmap_obs)
{
  ...
  for (const auto &obs : seg_obs)
  {
    ...
    if (is_valid_ellipse(ellipse) && ellipse.semimajor <= 20 && ellipse.semiminor <= 20)
...
void visEllipse(const std::vector<Ellipse> &obs_ellipses, ros::Publisher &_Ellipses_vis_pub)
{
  ...
  {
    if (!is_valid_ellipse(obs_ellipses[i]))
      continue;
